...
export class AppComponent {
  ...
  // this.description = data.data[0].data.data[0].tag;
  // this.analytics = data.data[0].analytics;
  


  ngOnInit() { 
    if(!localStorage.getItem('loadpopup')){
      localStorage.setItem('loadpopup','1'); 
    }    
    this.getMeta();
  }

  getMeta() { 
    this.api.get('seo/getMeta').subscribe((data: any) => {        
      if (data && data.status === 200 && data.data && data.data.length) {              
        //  this.tag = data.data[0].tag;
          ...
          // ]);
          // document.write(this.analytics);
          var string = this.analytics,
  ...
  navigationInterceptor(event: RouterEvent): void {
    ...
    if (event instanceof NavigationEnd) {
      ...
      this.titleService.setTitle(data && data[0] ? this.util.getString(data[0]) + ' | Ellvin Fresh' :
        this.util.getString('Home') + ' | Ellvin Fresh');
    }
          ...
          this.api.get('users/getDefaultSettings').subscribe((data: any) => {
            ...
            if (data && data.status === 200 && data.data) {
              ...
              if (language) {
                ...
                this.titleService.setTitle(trl && trl[0] ? this.util.getString(trl[0]) + ' | Ellvin Fresh' :
                  this.util.getString('Home') + ' | Ellvin Fresh');
              }
