...
export class ChangeNumberComponent implements OnInit {
  ...
  goToHome() {
    if (!this.mobile) {
      this.util.toast('error', this.util.getString('Error'), this.util.getString('mobile number is required'));
      return false;
    }
    const param = {
    ...
    this.util.start();
    this.api.post('users/changeNumber', param).subscribe((data: any) => {
      this.util.stop();
      console.log(data);
      if (data && data.status === 200) {
        if (this.util.twillo === '1') {
          ...
          Toast.fire({
            icon: 'success',
            title: this.util.getString('Number updated successfully')
          });
