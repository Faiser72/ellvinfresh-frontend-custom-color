/*
  ...
  Created : 1-jan-2021
  This App Template Source code is licensed aas per the
  terms found in the Website https://pvgroups.com/license
  Copyright and Good Faith Purchasers © 2020-present pvgroups.
*/
import { Component, OnInit, ViewChild, ChangeDetectorRef, ElementRef,TemplateRef } from '@angular/core';
...
export class HomeComponent implements OnInit {
  ...
  acceptcookies() {
    ...
    this.cityavailable = localStorage.getItem('cityavailable');
    if(localStorage.getItem('loadpopup') == '1'){
      this.modalService.open(this.privacyPopup, {centered: true,size: 'sm',ariaLabelledBy: 'modal-basic-title',backdrop: 'static' }).result.then((result) => {
        this.closeResult = `Closed with: ${this.reload()}`;
      }, (reason) => {
  ...
  reload(){
    localStorage.setItem('loadpopup','2');
    location.reload();
    location.reload();
  }
  ngOnInit(): void {
  ...
  subCate(item) {
    ...
    let sub_link_name = 'order-'+item.name.toLowerCase()+'-online'
    this.router.navigate([sub_link_name] ); 
  }
  ...
  goToSingleProduct(product) {
    ...
    // this.router.navigate(['product-detail'], param);
    localStorage.setItem('sub_cat_id',product.id);
    let name = 'order-'+product.name.toLowerCase().replace(' ','-')+'-online';
    // let name = 'order-'+product.name.toLowerCase()+'-online'; 
    this.router.navigate([name+'/'+product.name.toLowerCase().replace(' ','-')]);
  }
