...
export class CheckoutComponent implements OnInit {
  ...
  @ViewChild('couponValue') couponValue: ElementRef;
  selected: any = 1;
  ...
  RAZORPAY_OPTIONS = {
    ...
    'name': 'Ellvin Fresh',
    'order_id': '',
    'description': 'Ellvin Fresh Payment',
  ...
  getName() {
    return this.util.userInfo && this.util.userInfo.first_name ? this.util.userInfo.first_name + ' ' +
      this.util.userInfo.last_name : 'Ellvin Fresh';
  }

  getEmail() {
    return this.util.userInfo && this.util.userInfo.email ? this.util.userInfo.email : 'info@EllvinFresh.com';
  }
  ...
  flutterPay() {
    ...
    FlutterwaveCheckout({
      ...
      customizations: {
        title: 'Ellvin Fresh',
        description: 'Ellvin Fresh order',
  ...
  getOffers() {
    this.api.get('offers/single').subscribe((data: any) => {
      console.log(data);
  ...
  addcard() {
    ...
    this.api.externalPost('https://api.stripe.com/v1/tokens', param, this.util.stripe).subscribe((data: any) => {
      ...
      if (data && data.id) {
        // this.token = data.id;
        const customer = {
          description: 'Customer for Ellvin Fresh app',
          source: data.id,
          email: this.email
        };
  ...
  selectedOffers(item) {
      ...
      const param = {
        coupon: this.couponValue.nativeElement.value
      };
      this.util.start();
    this.api.post('users/validateCouon', param).subscribe((data: any) => {
      console.log(data);
      // return false;
      this.util.stop();
      if (data && data.status === 200) {        
        const info = data.data;
        const min = parseFloat(info[0].min);
        ...
        if (info && info[0].status == "1") {
          ...
          this.util.toast('error', this.util.getString('Error'), this.util.getString('Coupon is not Valid'));
        }
      } else {
        this.util.toast('error', this.util.getString('Error'), this.util.getString('Something went wrong'));
      }
